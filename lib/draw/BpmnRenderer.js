...
function BpmnRenderer(eventBus, styles, pathMap, canvas, priority) {
  ...
  function renderEmbeddedLabel(parentGfx, element, align) {
    return renderLabel(parentGfx, self.getLabelForElement(element), {
      box: element,
      align: align,
      padding: 5,
      style: {
  ...
  var handlers = this.handlers = {
    ...
    'bpmn:Participant': function(parentGfx, element) {
      ...
      if (expandedPool) {
        ...
        renderLaneLabel(parentGfx, self.getLabelForElement(element), element);
      } else {
        // Collapsed pool draw text inline
        renderLabel(parentGfx, self.getLabelForElement(element), {
    ...
    'bpmn:TextAnnotation': function(parentGfx, element) {
      ...
      renderLabel(parentGfx, self.getLabelForElement(element), { box: element, align: 'left-top', padding: 5 });
...
/**
 * Returns the label to render in the diagram for the given element
 * @param element Element
 * @return {string} Label for the given element
 */
BpmnRenderer.prototype.getLabelForElement = function(element) {
  if (is(element, 'bpmn:TextAnnotation')) {
    return getSemantic(element).text || '';
  } else {
    return getSemantic(element).name || '';
  }
};
